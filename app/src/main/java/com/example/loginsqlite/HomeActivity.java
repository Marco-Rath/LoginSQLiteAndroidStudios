    ...
    public class HomeActivity extends AppCompatActivity {
        ...
        EditText cajaMonto,cajaFecha,cajaIdTransferencia;
        ...
        protected void onCreate(Bundle savedInstanceState) {
            ...
            cajaIdTransferencia=findViewById(R.id.cajaIdTransferencia);
    ...
    btnBuscar.setOnClickListener(new View.OnClickListener() {
        ...
        public String getTransactionHistory() {
            DBHelper dbHelper = new DBHelper(this);
            int loggedUserId = Integer.parseInt(cajaId.getText().toString());
            Cursor cursor = dbHelper.getAllTransactionsForUser(loggedUserId);
            ...
            while (cursor.moveToNext()) {
                ...
                int transactionUserIdIndex = cursor.getColumnIndex("user_id");
                if (typeIndex!= -1 && amountIndex!= -1 && dateIndex!= -1 && transactionUserIdIndex!= -1) {
                    String type = cursor.getString(typeIndex);
                    double amount = cursor.getDouble(amountIndex);
                    String date = cursor.getString(dateIndex);
                    String transactionUserId = cursor.getString(transactionUserIdIndex); // Cambiado el nombre de la variable aquí
                    String lineFormat = "%-10s %-10s %-10s %-10s %-10s\n";
                    // Obtener el nombre del usuario usando el userId
                    String userName = dbHelper.getUserNameById(transactionUserId); // Asegúrate de usar la variable correcta aquí

                    // Formatear la línea de la transacción
                    transactionHistory.append(String.format(lineFormat, type, amount, date, userName, transactionUserId));
                }
            }
            cursor.close();
